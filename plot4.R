...
#makePlot4 makes the requested plot4 in a png file
makePlot4 <- function(dat=readin()){
  png(filename="plot4.png",width=480,height=480,units="px",bg="transparent")
  par(mfrow= c(2,2), mar=c(5,4,3,2))
  #subfigure row 1 col 1
  with(dat,plot(Date.Time,Global_active_power,type="l",xlab="",ylab="Global Active Power (kilowatts)"))
  #subfigure row 1 col 2
  with(dat,plot(Date.Time,Voltage,type="l",xlab="datetime",ylab="Voltage"))
  #subfigure row 2 col 1
  ...
  #subfigure row 2 col 2
  with(dat,plot(Date.Time,Global_reactive_power,type="l",xlab="datetime",ylab="Global_reactive_power"))
  dev.off()
}